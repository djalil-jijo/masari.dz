  ...
  return (
    <div className="max-w-6xl mx-auto p-6 font-arabic animate-fade-in">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-black text-primary-950  mb-4">أداة مقارنة التخصصات الجامعية</h2>
        <p className="text-slate-800  font-bold text-lg">اختر تخصصين للمقارنة بينهما بناءً على توافقهما مع شخصيتك وقدراتك</p>
      </div>
          ...
          <button
            ...
            className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2
              ${selectedMajors.includes(major.id) 
                ? 'border-primary-600 bg-primary-500/10  text-primary-600 shadow-lg shadow-primary-600/10' 
                : 'glass-morphism border-white/20 text-slate-700 hover:text-primary-600 hover:border-primary-200'
              } disabled:opacity-30 disabled:cursor-not-allowed`}
          >
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${selectedMajors.includes(major.id) ? 'bg-primary-600' : 'bg-white/60 border border-white/20'}`}>
              {selectedMajors.includes(major.id) && <CheckCircle2 size={16} className="text-white" />}
            </div>
            <span className="text-xs font-bold text-center leading-tight ">{major.name.split(' (')[0]}</span>
          </button>
        ))}
      </div>
            ...
            {majorData.map((major, idx) => (
              <div key={major.id} className="glass-morphism rounded-3xl p-8 border border-white/40 shadow-xl relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-8">
                   <h3 className="text-2xl font-bold text-primary-950  max-w-[200px]">{major.name}</h3>
                   <div className="text-right">
                      <div className="text-sm font-bold text-slate-700 ">توافقك</div>
                ...
                <div className="space-y-8">
                  <section>
                    <h4 className="text-sm font-bold text-slate-600  mb-3 uppercase tracking-wider italic">الوصف المختصر</h4>
                    <p className="text-slate-800  text-sm font-bold leading-relaxed">{major.description}</p>
                  </section>

                  <section>
                    <h4 className="text-sm font-bold text-slate-600  mb-3">متطلبات الميول (Holland)</h4>
                    <div className="flex flex-wrap gap-2">
                       {major.hollandTypes.map(h => (
                         <span key={h} className={`px-3 py-1 rounded-full text-xs font-bold glass-morphism border border-indigo-100/20 text-slate-700 `}>
                           {h === 'R' && 'واقعي'}
                           {h === 'I' && 'بحثي'}
                           {h === 'A' && 'فني'}
                           {h === 'S' && 'اجتماعي'}
                  ...
                  <section>
                     <h4 className="text-sm font-bold text-slate-600  mb-3">القدرات الذهنية المطلوبة</h4>
                     <ul className="space-y-2">
                        {major.requiredAbilities.map(a => (
                          <li key={a} className="flex items-center gap-2 text-sm text-slate-800  font-bold">
      ...
      {selectedMajors.length < 2 && (
        <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-slate-200  rounded-3xl text-slate-500">
