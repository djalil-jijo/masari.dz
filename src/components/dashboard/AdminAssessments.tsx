...
import { mockAssessments, AssessmentData } from '@/lib/data/mockAssessments';
...
export default function AdminAssessments() {
  ...
  const handleSave = () => {
    ...
    } else if (isEditModalOpen && selectedAssessment) {
      setAssessments(assessments.map(a => 
        a.id === selectedAssessment.id 
          ? { ...a, name: formName, type: formType } 
          : a
      ));
    }
  ...
  const openEditModal = (assessment: AssessmentData) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 glass-morphism p-6 rounded-3xl border border-white/40 shadow-sm">
        ...
        <button onClick={openAddModal} className="flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-xl font-bold shadow-lg shadow-primary-600/30 hover:bg-primary-700 transition-all w-full md:w-auto justify-center hover:scale-105 active:scale-95">
          <Plus size={18} />
          إضافة مقياس جديد
        </button>
      </div>
