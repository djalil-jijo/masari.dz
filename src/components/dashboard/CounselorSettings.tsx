  ...
  return (
    <div className="max-w-4xl mx-auto p-6 font-arabic animate-fade-in">
      <div className="mb-10">
        ...
        <p className="text-slate-700  font-medium">تخصيص لوحة التحكم وتفضيلات النظام</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="glass-morphism rounded-3xl border border-white/40 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-indigo-100/10 flex items-center gap-3">
             ...
             <h3 className="font-bold text-primary-950 ">إعدادات الإشعارات</h3>
          </div>
          <div className="p-6 space-y-4">
             {notifications.map((item) => (
               <div key={item.id} className="flex items-center justify-between py-2">
                  <div>
                    <div className="text-sm font-bold text-primary-950">{item.label}</div>
                    <div className="text-[10px] text-slate-600 font-bold">{item.desc}</div>
                  </div>
                 <button 
                   onClick={() => setNotifications(prev => prev.map(n => n.id === item.id ? { ...n, active: !n.active } : n))}
                   className={`w-12 h-6 rounded-full transition-all relative ${item.active ? 'bg-indigo-600' : 'bg-slate-200'}`}
                 >
        ...
        <div className="glass-morphism rounded-3xl border border-white/40 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-indigo-100/10 flex items-center gap-3">
             ...
             <h3 className="font-bold text-primary-950 ">المظهر والعرض</h3>
          </div>
          <div className="p-6 space-y-4">
             <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-3 font-bold text-slate-700 ">
                 ...
                 <button onClick={() => setDarkMode(!darkMode)} className={`w-12 h-6 rounded-full relative transition-colors ${darkMode ? 'bg-indigo-600' : 'bg-slate-200'}`}>
                    <div className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${darkMode ? 'left-1' : 'left-7'}`}></div>
                 </button>
             </div>
             <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-3 font-bold text-slate-700 ">
        ...
        <div className="bg-indigo-600 rounded-3xl p-8 text-white flex items-center justify-between shadow-lg shadow-indigo-600/30">
          <div className="space-y-2">
             ...
             <button onClick={() => alert('سيتم تحويلك إلى الدعم الفني...')} className="mt-4 px-6 py-2.5 bg-white text-indigo-600 rounded-xl font-bold text-sm hover:bg-indigo-50 transition-all flex items-center gap-2">
                <HelpCircle size={18} />
                تواصل مع الدعم
             </button>
          </div>
