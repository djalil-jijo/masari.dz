  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
        <div>
          <h2 className="text-3xl font-bold text-primary-950 dark:text-white mb-2">خطة تطوير المهارات</h2>
          <p className="text-slate-700 dark:text-slate-300 font-medium">خارطة طريق مخصصة لبناء قدراتك الشخصية والأكاديمية</p>
        </div>
        <div className="flex items-center gap-2 px-4 py-2 bg-primary-50 dark:bg-primary-900/30 text-primary-600 rounded-xl font-bold text-sm">
          <Target size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {skillsPlan.map((category, idx) => (
          <motion.div 
            ...
            className={`p-6 rounded-3xl glass-morphism border border-white/40 transition-all hover:shadow-xl dark:bg-slate-950/40`}
          >
            ...
            <div className="space-y-4">
              {category.skills.map((skill, sIdx) => (
                <div 
                  key={sIdx}
                  className="flex items-center justify-between p-4 glass-morphism/40 rounded-2xl border border-white/20 hover:bg-white/60 dark:hover:bg-slate-800 transition-colors cursor-pointer group shadow-sm"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-white dark:bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 border border-slate-100 dark:border-slate-600">
                      {sIdx + 1}
                    </div>
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="flex gap-4 items-start bg-white/5 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
              ...
              <p className="text-sm text-slate-300">ابدأ بمهارات التنظيم الزمني فهي القاعدة الأساسية للنجاح الأكاديمي.</p>
            </div>
            <div className="flex gap-4 items-start bg-white/5 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
              <MessageSquare className="text-green-400 shrink-0" size={20} />
              <p className="text-sm text-slate-300">ممارسة التواصل الفعال في الأنشطة الصفية ستزيد من ثقتك بنفسك.</p>
            </div>
            <div className="flex gap-4 items-start bg-white/5 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
              <Zap className="text-amber-400 shrink-0" size={20} />
              <p className="text-sm text-slate-300">خصص وقت للإبداع والابتكار خارج المنهاج الدراسي لتنمية روح المبادرة.</p>
            </div>
          </div>
        </div>
